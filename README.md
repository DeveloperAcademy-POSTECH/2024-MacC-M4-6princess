# 🎀 Frameet 🎀 언제 어디서나 최애와 함께 사진 찍기


## 🎯 Challenge Statement

아이돌을 좋아하는 팬들이 **좋아하는 연예인과 함께 찍은 것 같은 사진을 쉽게 만들 수 있도록 돕자.**
<!--


## 🔍 Refined Challenge

“**좋아하는 연예인과 사진을 찍고 싶은 팬이 최애와 자연스럽게 같이 찍은 것 같은 사진을 받아볼 수 있게 도와주자!**”

-->



## 👥 Target Users

- 아이돌을 좋아하는 모든 팬  
- 특히 **포토부스에서 네 컷 사진을 찍어본 경험이 있는 팬**  

<!--


## 🔎 User Research

### 🔬 Interview 1: 팬 문화 전반 파악  
- **목표**: 팬들의 활동 흐름과 니즈 확인  
- **결과**: 앱을 통한 문제 해결이 유의미할 수 있다는 판단

### 🔬 Interview 2: 포토부스 문화 내 문제 확인  
- **목표**: 팬들이 사진 찍을 때 겪는 구체적 불편 조사  
- **결과**: 포즈 제한, 프레임 문제, 장소 접근성 등의 문제 도출

### 🔬 Interview 3: 팬들의 구체적인 니즈 분석  
**문제점 정리**  
1. 아이돌 프레임 속에 내 몸을 맞추기 어려움  
2. 아이돌 포즈가 어색하면 나도 포즈 잡기 힘듦  
3. 지방 거주자는 포토부스 접근 어려움  
4. 비싼 가격(7,000원)에도 만족도 낮음  
5. 재촬영 횟수가 제한적이라 부담  

**기회 포착**  
→ 팬들은 실제로 **같이 찍은 듯한 자연스러운 결과물**을 원함
-->


## 💡 Solution Concept

> 팬이 아이돌 사진을 불러오고,  
> 자신이 원하는 사진을 넣어 **자동 배경 제거 + 프레임 조합**으로  
> **마치 아이돌과 함께 찍은 듯한 결과물**을 만들 수 있는 앱!



## 📱 핵심 기능 소개

| 순서 | 기능 |
|------|------|
| 1 | 다양한 아이돌 프레임 선택 |
| 2 | 사진 선택 & 배경 자동 제거 |
| 3 | 스티커, 텍스트로 꾸미기 |
| 4 | 사진 저장 및 공유 가능 |
| 5 | 무제한 촬영 & 집에서도 자유롭게 가능 |

<!--


## 🧪 주요 문제 정의 및 사용자 인사이트

| 사용자 발화 | 인사이트 |
|--------------|----------|
| “최애 프레임이 없으면 사진 못 찍어요.” | 프레임 다양성과 개인화 필요 |
| “프레임이 나랑 안 맞아요.” | 사이즈/포즈/이질감 문제 존재 |
| “집에서는 촬영이 안 돼요.” | 장소 제약 해결 필요 |
| “가격이 너무 비싸요.” | 비용 부담 해결 가능성 |
| “사람 많은 곳에서 찍기 창피해요.” | **온라인화**로 심리적 장벽 낮추기 |
-->
<!--


## 🧠 새로운 유저 등장 (MVP 이후 발견)

투명 포토카드 문화에서도 **“인증샷 찍기 어려움”** 이 존재  
→ “사람 많은 데서 예쁘게 못 찍겠어요”, “포토샵 너무 어려워요”, “재고가 너무 많이 남아요”  
→ **디지털로 인증샷 제작이 가능한 방식 필요**
-->


## 🖥️ Main Flow


![6@3x](images/screenshot1.png)
![7@3x](./images/screenshot2.png)




## 👩‍💻 Team 6공주 소개

<div align="left">

<table>
  <tr>
    <td align="center">
        <a href="https://www.linkedin.com/in/minkimskylar">
      <img src="/images/skyla.png" width="88" alt="Skylar"/>
        </a><br />
      <a href="https://www.linkedin.com/in/minkimskylar"><b>Skylar</b></a><br />PM
    </td>
    <td align="center">
        <a href="https://www.linkedin.com/in/yurimbae">
      <img src="/images/nana.png" width="88" alt="Nana"/>
        </a><br />
     <a href="https://www.linkedin.com/in/yurimbae"> <b>Nana</b></a><br />디자이너
    </td>
    <td align="center">
        <a href="https://www.linkedin.com/in/yejin-kang-a8b534209/">
      <img src="/images/sora.png" width="88" alt="Sora"/>
        </a><br />
      <a href="https://www.linkedin.com/in/yejin-kang-a8b534209"><b>Sora</b></a><br />디자이너
    </td>
    <td align="center">
        <a href="https://github.com/darongzzang">
      <img src="/images/jane.png" width="88" alt="Jane"/>
        </a><br />
     <a href="https://github.com/darongzzang"> <b>Jane</b></a><br />iOS 개발
    </td>
    <td align="center">
        <a href="https://github.com/TakeMos">
      <img src="/images/jamie.png" width="88" alt="Jamie"/>
        </a><br />
      <a href="https://github.com/TakeMos"><b>Jamie</b></a><br />iOS 개발
    </td>
    <td align="center">
  <a href="https://github.com/piriram">
    <img src="/images/piri.png" width="88" alt="Piri"/>
  </a><br />
 <a href="https://github.com/piriram"> <b>Piri</b></a><br />iOS 개발
</td>

  </tr>
</table>

</div>



## 📌 공주들의 목표

1. 앱으로 돈벌기! 
2. 사용자 인터뷰와 테스트를 가능한 자주, 깊이있게 진행하기!
3. 아카데미 이후로도 지속적으로 유지·관리하기!

## ⬇ 프레임밋 실행
[![Download on the App Store](https://developer.apple.com/assets/elements/badges/download-on-the-app-store.svg)](https://apps.apple.com/kr/app/frameet-%ED%94%84%EB%A0%88%EC%9E%84%EB%B0%8B/id6737822930)


